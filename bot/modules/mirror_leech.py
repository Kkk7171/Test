...
from bot import bot, DATABASE_URL, DATABASE_NAME, DOWNLOAD_DIR, LOGGER, config_dict, bot_name, categories_dict, user_data
...
#from bot.modules.autoleecher import thumbs
...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    if not is_url(link) and not is_magnet(link) and not await aiopath.exists(link) and not is_rclone_path(link) and file_ is None:
        btn = ButtonMaker()
        btn.ibutton('Cʟɪᴄᴋ Hᴇʀᴇ Tᴏ Rᴇᴀᴅ Mᴏʀᴇ ...', f'thiruml {message.from_user.id} help MIRROR')
...
async def thirumlcb(_, query):
    ...
    if user_id != int(data[1]):
        ...
        try:
            ...
            endLine = "\n----------<b>END LOG</b>----------"
            btn = ButtonMaker()
            btn.ibutton('Cʟᴏsᴇ', f'thiruml {user_id} close')
        ...
        await query.answer(url=f"https://t.me/{bot_name}?start=thiruml")
    elif data[2] == "help":
        await query.answer()
        btn = ButtonMaker()
        btn.ibutton('Cʟᴏsᴇ', f'thiruml {user_id} close')
        if data[3] == "CLONE":
            ...
            if len(data) == 4:
                ...
                btn.ibutton('Nᴇxᴛ Pᴀɢᴇ', f'thiruml {user_id} help MIRROR readmore')
            else:
                msg = MIRROR_HELP_MESSAGE[1][4000:]
                btn.ibutton('Pʀᴇ Pᴀɢᴇ', f'thiruml {user_id} help MIRROR')
            await editMessage(message, msg, btn.build_menu(2))
        if data[3] == "YT":
            await editMessage(message, YT_HELP_MESSAGE[1], btn.build_menu(1))
    elif data[2] == "guide":
        btn = ButtonMaker()
        btn.ibutton('Bᴀᴄᴋ', f'thiruml {user_id} guide home')
        btn.ibutton('Cʟᴏsᴇ', f'thiruml {user_id} close')
        if data[3] == "basic":
            ...
            buttons = ButtonMaker()
            buttons.ibutton('Basic', f'thiruml {user_id} guide basic')
            buttons.ibutton('Users', f'thiruml {user_id} guide users')
            buttons.ibutton('Mics', f'thiruml {user_id} guide miscs')
            buttons.ibutton('Owner & Sudos', f'thiruml {user_id} guide admin')
            buttons.ibutton('Close', f'thiruml {user_id} close')
...
async def clean_caption(text: str) -> str:
    if not text:
        return ""
    ...
    # === 2. Remove known spam/promo patterns ===
    blocked_domains = [
        r'1tamilmv\.se',
        r'1tamilmv\.\w+',
        r'tamiltwist\.\w+',
        ...
        r'tamilyogi\.\w+',
        r'movierulz\.\w+',
        r'\w+\.(se|in|cc|pw|xyz|icu|live|fun|buzz)'
    ]
    ...
    # === 6. Extract and return clean title ===
    for line in text.splitlines():
        line = line.strip()
        if len(line) > 20:
            # Remove first hyphen if it exists
            if line.startswith('-'):
                line = line[1:].strip()
            return line
...
async def ruto_rename_thiruxd(client, message):
   ...
   # reply_msg = await message.reply_text(f"/qbleech -n {clean_text}.mkv")
    if not data:
        ...
        reply_msg = await message.reply_text(f"/qbleech -n {clean_text}.mkv -t {data['url']}")
    _mirror_leech(client, reply_msg, isQbit=True, isLeech=True)
    await sleep(5)
