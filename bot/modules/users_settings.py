...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            'lprefix': ['Leech Filename Prefix is the Front Part attacted with the Filename of the Leech Files.', 'Send Leech Filename Prefix. Documentation Here : <a href="https://t.me/ThiruEmpire/114">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lsuffix': ['Leech Filename Suffix is the End Part attached with the Filename of the Leech Files', 'Send Leech Filename Suffix. Documentation Here : <a href="https://t.me/ThiruEmpire/114">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lremname': ['Leech Filename Remname is combination of Regex(s) used for removing or manipulating Filename of the Leech Files', 'Send Leech Filename Remname. Documentation Here : <a href="https://t.me/ThiruEmpire/114">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lcaption': ['Leech Caption is the Custom Caption on the Leech Files Uploaded by the bot', 'Send Leech Caption. You can add HTML tags. Documentation Here : <a href="https://t.me/ThiruEmpire/114">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            ...
            'usess': [f'User Session is Telegram Session used to Download Private Contents from Private Channels with no compromise in Privacy, Build with Encryption.\n{"<b>Warning:</b> This Bot is not secured. We recommend asking the group owner to set the Upstream repo to the Official repo. If it is not the official repo, then Thiru-ML is not responsible for any issues that may occur in your account." if config_dict["UPSTREAM_REPO"] != "https://github.com/ThiruXD/Thiru-ML" else "Bot is Secure. You can use the session securely."}', 'Send your Session String.\n<b>Timeout:</b> 60 sec'],
            ...
            'metadata': ['Your channel name that should be used while editing metadata of the file', 'Send Leech Filename Metadata.\n<b>Timeout:</b> 60 sec'],
            }
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        text = BotTheme('LEECH', NAME=name, DL=f"{dailyll} / {dailytlle}",
                ...
                LSUFFIX=escape(lsuffix), LDUMP=ldump, LREMNAME=escape(lremname),  METADATA=escape(metadata))
        ...
        elif key in ['lprefix', 'lremname', 'lsuffix', 'lcaption', 'ldump', 'metadata']:
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'yt_opt', 'metadata'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await query.answer()
        buttons = ButtonMaker()
        buttons.ibutton('Cʟᴏsᴇ', f'thiruml {user_id} close')
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlsuffix', 'dlremname', 'dlcaption', 'dldump', 'dmetadata']:
        handler_dict[user_id] = False
        await query.answer()
