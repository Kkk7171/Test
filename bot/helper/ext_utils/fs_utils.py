...
async def eedit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ""):
    cmd = [
        ...
        "-metadata", f"Encoded by={metadata}",

        # Video
        "-metadata:s:v", f"title={metadata}",
        "-metadata:s:v", f"AUTHOR={metadata}",
        "-metadata:s:v", f"Encoded by={metadata}",

        # Audio
        "-metadata:s:a", f"title={metadata}",
        "-metadata:s:a", f"AUTHOR={metadata}",
        "-metadata:s:a", f"Encoded by={metadata}",

        # Subtitle
        "-metadata:s:s", f"title={metadata}",
        "-metadata:s:s", f"AUTHOR={metadata}",
        "-metadata:s:s", f"Encoded by={metadata}",
    ...
    if await listener.suproc.wait() == 0:
        ...
        await clean_target(outfile)
        LOGGER.error("Metadata update failed: %s", (await listener.suproc.stderr.read()).decode())
